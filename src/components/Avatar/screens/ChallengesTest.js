...
export default class ChallengesTest extends Component{
    render(){
        return (
            <View style={styles.container}>
              ...
              <Button title="get Level" onPress={this.GetLevel}></Button>
            </View>
          )
    }
    AddMealPress(){
    ...
    async GetLevel(){
      var test = await AchievementManager.getCurrentLevel();
      console.log("current level", test);

      var test2 = await AchievementManager.getLevelPoints();
      console.log("levelpoints", test2);
    }
